...
- list selection UI, current actions, lifecycle rules, policies, shared roles.
...
- select items.
- show selected count.
- perform action in scoped authorized batch.
- report affected/skipped/failed counts.
...
- Flux bulk toolbar, confirmation modal, progress/result toast.
...
- never trust submitted IDs.
- authorize every selected item.
- reject or skip unauthorized items as documented.
...
- bulk action own data test.
- bulk cannot affect other user.
- role restrictions.
...
- Step 039 is individually completed and not grouped with other steps.
