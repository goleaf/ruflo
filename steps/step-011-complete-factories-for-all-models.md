...
- every existing model, relationship, enum/status, lifecycle state, ownership field, collaboration scope, and current factory coverage.
...
- add named factory states for active, completed, archived, deleted, overdue, today, upcoming, private, shared, invalid-edge, heavy-user, empty-user, demo-user.
- ensure factories create valid related models without hidden global leakage.
...
- factory data must support UI testing for lists, dashboards, filters, empty states, comments, attachments, invites, and settings.
...
- factories must support at least two users to test privacy.
- shared factories must clearly distinguish owner, member, viewer, editor, manager, removed user.
...
- test each factory can create a valid record.
- test related factories create scoped data.
- test factory states do not create invalid ownership by accident.
...
- Step 011 is individually completed and not grouped with other steps.
