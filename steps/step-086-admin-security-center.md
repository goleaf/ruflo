...
- existing admin concepts, owner user, environment flags, maintenance tools, health checks.
- CSRF, XSS, policies, mass assignment, files, imports, exports, invites, cache, logs, debug, public routes.
...
- protected owner/admin center.
- health/storage/process/failure summaries.
- safe demo reset if local only.
- no secret exposure.
- fix critical issues.
- tighten validation/authorization.
- remove debug output.
...
- Flux admin panels, status badges, dangerous action confirmations.
- safe errors.
...
- no sensitive server paths/secrets.
- demo reset never production.
- complete security checklist.
- no known critical leaks remain.
...
- normal user denied.
- production demo reset hidden.
- security regression tests for fixed issues.
- privacy tests.
...
- Step 086 is individually completed and not grouped with other steps.
