...
- tag model, assignment logic, duplicate handling, ownership, tag filters/search.
...
- validate names.
- handle duplicates predictably.
- bulk tag assignment if needed.
...
- Flux badges, tag selector, tag filters, empty tag state.
...
- no global private tag leak.
- cannot assign another user's tag.
...
- tag creation/assignment/filter tests.
- multi-user tag isolation tests.
...
- Step 029 is individually completed and not grouped with other steps.
