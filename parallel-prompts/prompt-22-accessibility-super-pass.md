# DESIGN PROMPT 22 — Accessibility super pass

## Goal

Perform a full accessibility pass across the app.
...
Focus on accessibility improvements. Do not redesign entire layouts unless accessibility requires it.

## Inspect before changing

Inspect headings, labels, button names, icon-only controls, focus states, modals, dropdowns, tables/lists, forms, alerts, toasts, status badges, comments, file uploads, settings.

## Required implementation

Fix missing labels, aria names, focus order, keyboard traps, modal focus, contrast issues, non-color indicators, screen reader status text, and error associations.

## Mobile requirements

Touch targets and focus states must work on mobile.

## Tablet requirements

Keyboard and touch hybrid behavior must remain sane.

## Desktop requirements

Keyboard navigation must work across menus, dropdowns, modals, forms, and task actions.
...
Run accessibility-related tests if available and document manual keyboard/screen-reader checks.

## Required report

Create or update:

`DESIGN_AGENT_REPORT_22.md`
