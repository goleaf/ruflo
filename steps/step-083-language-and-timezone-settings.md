...
- date storage/display, timezone settings, due date filters, reminder logic, recurrence, dashboard.
- profile/settings, preferences, workspace roles, notification settings, dashboard settings.
...
- clear date logic for no due date, today, overdue, upcoming.
...
- reset options.
- activity for important workspace changes.
...
- due date badges, date inputs, today/overdue/upcoming empty states.
- Flux settings panels/tabs, helper text, danger zone, confirmations.
...
- date filters must remain scoped.
- invalid date input must not cause unsafe query.
- user cannot change another user's settings.
- workspace settings role-protected.
- removed members denied.
...
- today/overdue/upcoming tests.
- completed/archived exclusions.
- timezone tests where supported.
- personal settings.
...
- Step 083 is individually completed and not grouped with other steps.
