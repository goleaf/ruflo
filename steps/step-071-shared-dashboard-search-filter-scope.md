...
- user model, workspace/team/org concepts, task ownership, route model binding, query scopes, policies, dashboard/search/filter code.
- current lists, query scopes, filter params, sorting params, pagination, saved state.
- existing dashboard, queries, widgets, filters, preferences, performance.
- workspace/project model, roles, policies, current sharing assumptions.
...
- make private workspace default.
- assign ownership in backend.
- scope lists, dropdowns, counts, dashboards, search, filters, notifications, activity, files, exports, settings.
- avoid global records unless intentionally public.
- private scoped search/filter/sort/view behavior.
- validate query params.
- combine with pagination.
- support reset/clear.
- accurate widgets.
- private scoped counters.
- optimized query structure.
- empty/loading states.

## Livewire and Flux UI requirements

- show only current user's workspace resources.
- safe empty states when no data.
- no UI dropdown may include another user's resources.
- Livewire debounced search.
- Flux filter panel.
- active filter chips.
- empty result state.
...
- responsive grid.
- widget settings.
- accessible summaries for charts.
- member list, role badges, shared/private indicators, access denied states.
...
- backend denies guessed IDs.
- queries must not load all records then hide unauthorized in UI.
- caches must be user/workspace scoped.
- search/filter must not reveal another user's record names/counts.
- invalid sorting cannot inject query logic.
- dashboard must not count/show unauthorized data.
- cache keys user/workspace scoped.
- private by default.
- viewer cannot edit.
...
- two-user isolation tests for every touched resource.
- test dashboard/search/filter counts are private.
- search privacy tests.
- filter/sort validation tests.
- pagination with filters tests.
- widget accuracy.
- multi-user privacy.
- archived/deleted exclusions.
- performance review notes.
- viewer/editor/manager/owner tests.
- removed member old link tests.
...
- Step 071 is individually completed and not grouped with other steps.
