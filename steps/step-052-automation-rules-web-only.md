...
- all repeated validation logic, custom messages, existing language files, request classes, Livewire validation, and business rules.
- actions/events, reminders, recurrence, web-only limitations, maintenance center.
...
- create single-purpose custom rules for ownership, lifecycle transition, project ownership, tag ownership, invite token, invite status, recurrence rule, reminder time, file type, import format, export scope, bulk action selection, timezone, language, role, and shared access where needed.
- avoid one giant rule.
- make messages translatable.
- simple web-triggered automation rules.
- trigger from user actions or manual process.
- no cron dependency.
- show logs/status.
...
- use friendly English validation messages.
- show field names through translated attributes.
- avoid technical wording.
- automation rules list, enable/disable, test/run now, result report.
...
- rules must not leak whether another user's private record exists.
- rules must be scoped before checking sensitive details.
- automation must run only within user's scope.
- rules cannot change unauthorized resources.
...
- test every custom rule success/failure.
- test translated messages.
- test unauthorized IDs fail safely.
- rule trigger tests.
- disabled rule ignored.
- multi-user scope tests.
...
- Step 052 is individually completed and not grouped with other steps.
