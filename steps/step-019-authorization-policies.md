...
- existing policies, gates, role checks, controller authorization, Livewire authorization, and missing policy coverage.
...
- create/update policies for every resource.
- authorize view/create/update/delete/restore/archive/complete/reopen/bulk/export/import/manage actions.
- ensure Livewire actions call authorization before mutation.
...
- hide/disable actions user cannot perform, but do not rely on UI for security.
- show safe access denied states.
...
- prevent role escalation, owner spoofing, member removal abuse, unauthorized downloads, export/import abuse.
...
- policy tests for owner/non-owner.
- role tests for viewer/editor/manager/owner.
- backend forbidden request tests.
...
- Step 019 is individually completed and not grouped with other steps.
