...
test('database seeder creates safe demo users and complete private workspaces', function () {
    ...
    expect($users)->toHaveCount(2)
        ...
        ->and(Reminder::query()->count())->toBe(6)
    ...
    $users->each(function (User $user): void {
        expect($user->projects()->whereNull('archived_at')->count())->toBe(2)
            ...
            ->and($user->reminders()->count())->toBe(3)
            ->and($user->reminders()->due()->count())->toBe(1)
            ->and($user->reminders()->whereNotNull('skipped_at')->count())->toBe(1)
