...
- task statuses, project columns, existing lists, drag/drop capability, mobile fallback.
...
- Kanban view using Livewire.
- safe move between columns.
- fallback buttons if drag/drop is not reliable.
...
- moving card must authorize task and target state/project.
- shared role restrictions apply.
...
- mobile/fallback action works where testable.
...
- Step 040 is individually completed and not grouped with other steps.
