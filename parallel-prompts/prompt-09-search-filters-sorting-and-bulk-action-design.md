# DESIGN PROMPT 09 — Search, filters, sorting, and bulk action design

## Goal

Upgrade search/filter/sort/bulk UX across task-heavy screens.
...
Touch filter panels, search inputs, sorting controls, active chips, bulk toolbar, result states.

## Inspect before changing

Inspect filter query state, Livewire debounce, active filters, reset buttons, sorting controls, bulk selection, confirmation flows, mobile filter behavior.

## Required implementation

Create collapsible mobile filter panel, desktop persistent filters, active chips, clear reset, empty filtered state, selected count, safe bulk confirmations, and result reports.

## Mobile requirements

Filters must be in drawer/collapsible panel; search sticky or easy to reach; bulk toolbar must not cover content badly.

## Tablet requirements

Use side panel or collapsible region depending on space.

## Desktop requirements

Use efficient filter sidebar/topbar with active state visible.
...
Test filter state, reset behavior, bulk selected count, and unauthorized bulk actions remain blocked.

## Required report

Create or update:

`DESIGN_AGENT_REPORT_09.md`
