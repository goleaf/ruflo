...
- current database seeders, demo data assumptions, environment protections, and login/demo page needs.
...
- seed every model and state.
- seed multiple demo users with known fixed passwords.
- seed tasks in all lifecycle states.
- seed projects, tags, priorities, due dates, reminders, recurrence, activity, comments, attachments metadata, imports/exports, settings, invites, collaboration roles.
- make demo data rich enough to open every screen immediately.
...
- seed data must make dashboard, task lists, Kanban, calendar, comments, attachments, reports, and settings visually testable.
...
- demo seed/reset tools must be local/testing/demo-only.
- never overwrite production data unless explicitly protected and confirmed.
...
- test seeders complete successfully.
- test demo users exist.
- test login page displays seeded demo users locally.
- test seeded private data remains separated.
...
- Step 012 is individually completed and not grouped with other steps.
