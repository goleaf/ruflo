...
- controllers, Livewire components, route actions, existing FormRequest classes, inline validation arrays, duplicated rules, custom rules, and validation messages.
...
- create dedicated request classes for create/update/delete/archive/restore/complete/reopen/invite/comment/attachment/import/export/settings/bulk/reminder/recurrence actions where appropriate.
- move authorization logic into request authorize method when request-driven.
- keep simple Livewire-only validation inside components only when truly component-specific.
- extract reusable validation out of fat components.
...
- validation errors must display near Flux inputs and in summary where useful.
- failed validation must preserve user input.
...
- request classes must reject ownership spoofing, unexpected IDs, invalid state transitions, dangerous file/input, and unauthorized role changes.
...
- test request rules for valid and invalid input.
- test authorize denies forbidden users.
- test translated messages appear.
...
- Step 014 is individually completed and not grouped with other steps.
