...
- frontend assets, manifest, service worker presence, cache risks.
...
- PWA basics only with private-data-safe caching.
- offline fallback.
- installability.
...
- offline page, install hint if appropriate, no broken private UI.
...
- do not cache private pages/files unsafely.
- logout clears sensitive cached state where possible.
...
- private data not intentionally cached.
...
- Step 089 is individually completed and not grouped with other steps.
