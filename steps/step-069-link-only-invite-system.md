...
- existing invitation logic, email notifications, mail dependencies, invite tokens, role assignment, expiration/cancellation behavior, routes, and tests.
...
- remove email invite dependency.
- generate copyable invite links in UI.
- support invite cancellation.
- support expiration if existing/product rule says so.
- support accepted/used behavior.
- link must use https://ruflo.test/ through app URL config.
- show pending/accepted/cancelled/expired states.
...
- Flux modal/form for creating invite.
- copy button for link.
- status badges for invite states.
- clear warning that invite is shared manually, not emailed.
...
- invite link must not expose private data before acceptance.
- cancelled/expired/used links must not grant access.
- role/scope must be validated on accept.
- pending invite is not membership.
...
- test link generated.
- test no email is sent.
- test accept valid link.
- test cancelled link fails.
- test expired link fails.
- test link cannot grant wrong role/scope.
...
- Step 069 is individually completed and not grouped with other steps.
